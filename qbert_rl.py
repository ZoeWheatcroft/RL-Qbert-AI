...
class DQN(nn.Module):
    ...
    def __init__(self, n_observations, n_actions):
        ...
        self.layer2 = nn.Linear(128, 128) #processing 
        self.layer3 = nn.Linear(128, n_actions) #output

    # Called with either one element to determine next action, or a batch
    # during optimization. Returns tensor([[left0exp,right0exp]...]).
    def forward(self, x):
        x = torch.transpose(torch.flatten(x, 2), 1, 2)
...
parser.add_argument('-l', '--load', default="qbert.pytorch", help="file to load model from", type=str)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            return policy_net(state).max(1)[1].max(1)[1].view(1, 1) #CHANGED 1 TO 6
    else:
